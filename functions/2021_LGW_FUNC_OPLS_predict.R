...
lgw_opls <- function(
  ...
  # perform OPLS-DA prediction and extract plotly loadings
    opls_x_predict <- FUNC_data %>%
        filter(sample_type == "sample") %>%
        filter(sample_class == FUNC_OPLS_predict) %>%
        select(all_of(FUNC_metabolite_list)) %>% 
        as.matrix()+1 
    ...
    plot_Val_training$opls_colour <- c(opls_colour)
    plot_Val_training$opls_plot_label <- c(opls_plot_label)
    ...
    opls_colour_predict <- FUNC_data %>% 
      filter(sample_type == "sample") %>% 
      filter(sample_class_factor == FUNC_OPLS_predict) %>%
      select(all_of(FUNC_colour_by)) #%>% as.matrix()

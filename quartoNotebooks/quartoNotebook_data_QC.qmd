...
mstr_lst <- list(
  ...
  data = list(
    ...
    plate_lst = list.files(paste0(.$proj_optns$proj_dir, .$proj_optns$proj_data_subdir), full.names = FALSE),
...
calibratR <- function(FUNC_data,
 ...
 FUNC_list$cal_plot <- ggplot(
    FUNC_list$lmX$model, x = std_conc, y = get(FUNC_HEADER_cal),
    aes_string(x = names(FUNC_list$lmX$model)[2], y = names(FUNC_list$lmX$model)[1])) +
      stat_smooth(geom = "line", alpha = 0.5, method = "lm", col = "darkgreen", fullrange = TRUE, se = FALSE,
                mapping = aes(weight = `(weights)`)) +
    geom_point(aes(colour = "std_pass", shape = "std_pass", size = "std_pass")) +
    geom_point(data = filter(FUNC_list$lmX_pred_data, type == FUNC_STRING_std),
               aes(colour = "std_fail", shape = "std_fail", size ="std_fail")) +
    geom_point(data = filter(FUNC_list$qc_summary, pass == TRUE),
               aes(colour = "qc_pass", shape = "qc_pass", size = "qc_pass", name = name)) +
    geom_point(data = filter(FUNC_list$qc_summary, pass == FALSE),
              aes(colour = "qc_fail", shape = "qc_fail", size = "qc_fail", name = name)) +
    geom_point(data = (FUNC_list$lmX_pred_data %>% filter(type == FUNC_STRING_sample) %>%
                         select(-std_conc) %>%
                         rename(std_conc = calc_conc)),
               aes(colour = "study_sample", shape = "study_sample", size = "study_sample", name = name)) +
    scale_size_manual(values = c(2.5,2.5,2.5,2.5,1), guide = "none") +
    scale_shape_manual(values = c(25,25,21,21,3), name = "sample_type", 
                       labels = c("qc_fail", "qc_pass", "std_fail", "std_pass", "study_samples")) +
    scale_color_manual(values = c( "red", "darkgreen", "red", "darkgreen", "black"), name = "sample_type", 
                       labels = c("qc_fail", "qc_pass", "std_fail", "std_pass", "study_samples")) +
    labs(title = paste0(FUNC_STRING_title, "\n",
      "R2 = ",signif(summary(FUNC_list$lmX)$adj.r.squared, 4),
      "; Intercept = ",signif(FUNC_list$lmX$coef[[1]],2),
      "; Slope = ",signif(FUNC_list$lmX$coef[[2]], 2))) +
    theme_bw() +
...
# idx_metabolite = "(-)-Epinephrine"
# idx_metabolite = "tryptophan"
      #create calibration output list()
  mstr_lst$calib[[idx_metabolite]] <- lst(
  ...
  if(!is.na(mstr_lst$calib[[idx_metabolite]]$SIL_used)){
  #create response ratio
   mstr_lst$data$response[[idx_metabolite]] <- mstr_lst$data$area %>%
     select(plate, name, type, all_of(c(idx_metabolite, mstr_lst$calib[[idx_metabolite]]$SIL_used))) %>%
     add_column(std_conc = mstr_lst$data$std_conc[[idx_metabolite]], .before = idx_metabolite) %>%
     mutate_at(c("std_conc", idx_metabolite, mstr_lst$calib[[idx_metabolite]]$SIL_used), as.numeric) %>%
     mutate(response = .[[idx_metabolite]]/.[[mstr_lst$calib[[idx_metabolite]]$SIL_used]])
  
   #perform the below for each plate independantly
   for(idx_plate in 1:length(mstr_lst$data$plate_lst)){
   #run calibratR function
  mstr_lst$calib[[idx_metabolite]][[paste0("plate_", idx_plate)]] <- calibratR(
  ...
  mstr_lst$calib[[idx_metabolite]]$all_plates$plots[[idx_plate]] <-
    mstr_lst$calib[[idx_metabolite]][[paste0("plate_", idx_plate)]]$cal_plot
  
   mstr_lst$calib[[idx_metabolite]]$all_plates$summary <- mstr_lst$calib[[idx_metabolite]]$all_plates$summary %>%
     bind_rows(mstr_lst$calib[[idx_metabolite]][[paste0("plate_", idx_plate)]]$summary_tbl)
   
   mstr_lst$calib[[idx_metabolite]]$all_plates$lmX_pred_data <-
     mstr_lst$calib[[idx_metabolite]]$all_plates$lmX_pred_data %>%
     bind_rows(mstr_lst$calib[[idx_metabolite]][[paste0("plate_", idx_plate)]]$lmX_pred_data)
   }
   
   if(min(mstr_lst$calib[[idx_metabolite]]$all_plates$summary$r2) > 0.99){
   mstr_lst$data$calib <- mstr_lst$data$calib %>%
     left_join(., select(mstr_lst$calib[[idx_metabolite]]$all_plates$lmX_pred_data, name, all_of(idx_metabolite)), 
               by="name") %>%
...
# mstr_lst$calib$tryptophan$all_plates$plots[[1]] %>% 
#   plotly::ggplotly(tooltip = c("std_conc", "response", "colour", "name"))
# mstr_lst$calib$tryptophan$all_plates$summary %>%
#   kbl(align = "c", booktabs = TRUE, longtable = TRUE) %>%
#   kable_minimal("hover") %>%
#   kable_styling(font_size = 12) %>%
#   scroll_box() %>% print()

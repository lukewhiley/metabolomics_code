...
```{r calibration functions, eval=TRUE, echo=TRUE, paged.print=FALSE, warning=FALSE}
...
#median LTR conc
LTR_median = (FUNC_data %>% filter(type == "LTR"))[["calc_conc"]] %>% median()
#LTR std.dev 
LTR_sd = (FUNC_data %>% filter(type == "LTR"))[["calc_conc"]] %>% sd()
...
FUNC_plot <- ggplot(data = FUNC_data,
       aes(x=sample_idx, y=log(calc_conc+1), fill = type, shape=type, size=type)) +
...
```{r report - cal, eval=TRUE, echo=TRUE, paged.print=FALSE, warning=FALSE, results='asis'}
...
height = (plot_row_freq+2) * 400,
width = 800,
rowsize = c(plot_row_freq,1,1),
ncol = 1
)
...
```{r report - response, eval=TRUE, echo=TRUE, paged.print=FALSE, warning=FALSE, results='asis'}
...
```{r report - area, eval=TRUE, echo=TRUE, paged.print=FALSE, warning=FALSE, results='asis'}

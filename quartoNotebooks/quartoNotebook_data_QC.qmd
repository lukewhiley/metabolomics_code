...
#### load packages
package_list <- c('tidyverse', 'janitor', 'qualV', 'kableExtra', 'shiny', 'manipulateWidget')
...
#### load packages
package_list <- c('tidyverse', 'janitor', 'qualV', 'kableExtra', 'plotly')
...
```{r calibration of data, eval=TRUE, echo=TRUE, paged.print=FALSE, warning=FALSE}
...
calibratR <- function(FUNC_data,
  ...
  add_column(
    type_factor = factor(.$type, levels = c(unique(.$type)), ordered = TRUE))
    
  #create plot  
  FUNC_list$cal_plot <- ggplot(data = FUNC_list$plot_data,
                 aes(x=conc, y=response, color=type, shape=type, size=type)) +
  geom_point() +
  stat_smooth(data = filter(FUNC_list$plot_data, type == "std_pass"),
...
for(idx_metabolite in mstr_lst$data$SIL_template$standard){
  ...
  temp_lst$all_plates$plots[[idx_plate]] <-
   ...
   #append temp_list to mstr_lst$calib$conc
   mstr_lst$calib$area[[idx_metabolite]] <- temp_lst
   }
}
...
```{r report, eval=TRUE, echo=TRUE, paged.print=FALSE, warning=FALSE}

#mstr_lst$calib$conc$tryptophan$all_plates$plots
...
for(idx_metabolite in names(mstr_lst$calib$conc)){
  plot_freq <- length(mstr_lst$calib$conc[[idx_metabolite]]$all_plates$plots)
  plot_height <- (plot_freq+1) * 200
  mstr_lst$report$conc[[idx_metabolite]] <- combineWidgets(
    #combineWidgets for plots
    # combineWidgets(
    #   list=mstr_lst$calib$conc[[idx_metabolite]]$all_plates$plots,
    #   ncol = 3)
    mstr_lst$calib$conc[[idx_metabolite]]$all_plates$plots %>% 
      subplot(
        #nrows = 2
          ),
...
height = plot_height,
width = 800,
title = idx_metabolite,
titleCSS = "height: 20px; width: 100%; background-color: powderblue; font-size: 15px;",
rowsize = c(3,1)
)
  
  # mstr_lst$report$conc[[idx_metabolite]] <-
  #   mstr_lst$calib$conc$`3-hydroxyanthranilic acid`$all_plates$plots %>% subplot()
  # 
}

```

```{r create temp folders, warning=FALSE}

temp_rmd_list <- list()

for (idx_metabolite in names(mstr_lst$report$conc)) {

  temp_rmd_path <- paste0("/Users/lukegraywhiley/Downloads/temp_data/test_qc/rmd/", idx_metabolite, ".rmd")
...
```{r knit, results='asis', warning=FALSE}
...
htmltools::tagList(mstr_lst$report$conc$dopamine)
htmltools::tagList(mstr_lst$report$conc$kynurenine)
```

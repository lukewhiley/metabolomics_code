...
#' Following the creation of a response ratio with an appropiate internal standard (as pre-defined by the user) features are removed at this QC checkpoint if the percentage relative standard deviation (%RSD) was greater than 30% in the LTR QC samples
#'
#'  
#+ echo=FALSE, message=FALSE, fig.width=10, fig.height=4

print(paste(nrow(ltr_rsd)-length(which(ltr_rsd$RSD < 30)), " lipid targets had a LTR RSD of > 30% so were removed from the dataset", sep=""))
print(paste("Total number of lipid target response ratios with with an LTR RSD of <30% =", length(which(ltr_rsd$RSD < 30))))
print(paste("Total number of lipid target response ratios with with an LTR RSD of <20% =", length(which(ltr_rsd$RSD < 20))))
print(paste("Total number of lipid target response ratios with with an LTR RSD of <15% =", length(which(ltr_rsd$RSD < 15))))
print(paste("Total number of lipid target response ratios with with an LTR RSD of <10% =", length(which(ltr_rsd$RSD < 10))))
...
print(paste("PCA plot created using the",  length(which(ltr_rsd$RSD < 30)), "lipids in the final dataset" ))
pca_p[[1]][[1]]

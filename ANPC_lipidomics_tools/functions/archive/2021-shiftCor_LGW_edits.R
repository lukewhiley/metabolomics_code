...
#' @export 
shiftCor_LGW <- function(samPeno_LGW, samFile_LGW, Frule = 0.8, MLmethod = "QCRFSC", ntree = 500, QCspan = 0, 
  ...
  cat("\n", "Data Link", "\n")
  #cat(" metaFile:", samPeno_LGW, "\n")
  #cat(" profileFile:", samFile_LGW, "\n")
  ...
  samFP <- samFile %>% filter(name %in% samPeno$sample)
  ...
  colnames(dat) <- samFP$name  #LGW addition
  numX <- 1:dim(dat)[2]
